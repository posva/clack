...
const EXPECTED_BRANCH = 'feat/agent-mode'
// No "main" package at root; all packages use scoped tags like @posva/clack-core@1.0.0
const _MAIN_PKG_NAME = ''
const _IS_MAIN_PKG_AT_ROOT = false
// array of folders of packages to release
const PKG_FOLDERS = [
	join(__dirname, '../packages/core'),
	join(__dirname, '../packages/prompts'),
]
	...
	const releaseConfigs = packagesToRelease.map(({ name, pkg }) => {
		...
		const options = versionIncrements
			.map((release) => {
				...
				if (!newVersion) {
					throw new Error(`Failed to calculate version increment for ${name} (${release})`)
				}
				return {
					value: newVersion,
		...
		pkgWithVersions.map(async (pkg) => {
			...
			await runIfNotDry(
				...
				[
					...
					changelogExists ? '1' : '0',
					'--commit-path',
					// All packages in packages/ use `.` for conventional-changelog scope
					'.',
					'--lerna-package',
					pkg.name,
					'--tag-prefix',
